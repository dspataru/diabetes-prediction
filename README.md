...
This part through various data visualization techniques, specifically employing Univariate Analysis, Bivariate Analysis, and a Correlation Matrix. The goal is to identify and retain the most relevant features for subsequent machine learning tasks or data analysis. The feature selection process involves two distinct methods: Mutual Information and Correlation Coefficient.

For statistical operations and data manipulation, the project relies on Pandas, NumPy, SciPy, Category Encoders, and Scikit-learn. These libraries facilitate effective feature selection, ensuring the chosen features contribute valuable information to the analysis or model training. Additionally, Seaborn, Matplotlib, and Plotly are employed for creating insightful visualizations, aiding in the exploration of feature relationships.

Here is two main method we used in feature selection: 
The Mutual Information method is utilized to measure the dependency between features and the target variable, making it suitable for both categorical and continuous target variables. Implementation involves using the mutual_info_classif function from Scikit-learn to compute Mutual Information scores.


The Correlation Coefficient method assesses the linear relationship between features and the target variable. This method is particularly useful for continuous target variables. The implementation involves computing the correlation matrix using Pandas and selecting features with correlation coefficients above a specified threshold.
